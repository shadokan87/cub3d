...
cd /etc/nginx/ && mv nginx.conf nginx.conf.ORIG && mv /etc/php/7.3/fpm/php-fpm.con    f /etc/php/7.3/fpm/php-fpm.conf.ORIG && mv /etc/php/7.3/fpm/php.ini /etc/php/7.3/fpm/ph    p.ini.ORIG
mkdir -p /home/ft_server/logs /usr/share/nginx/cache/fcgi /var/run/php-fpm && rm /    etc/php/7.3/fpm/pool.d/www.conf
cp ./root/nginx.conf ./etc/nginx && cp ./root/php-fpm.conf ./etc/php/7.3/fpm/ && c    p ./root/www.conf ./etc/php/7.3/fpm/pool.d/www.conf && cp ./root/php.ini ./etc/php/7.3/    fpm/php.ini
bash root/restart_services.sh
